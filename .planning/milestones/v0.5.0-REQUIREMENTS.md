# Requirements Archive: v0.5.0 Auto-Update + Healthcheck
...
- [x] **WIN-01**: `conjure.ps1` PowerShell shim (≤30 lines) locates Git Bash or
...
- [x] **WIN-02**: CI matrix includes a `windows-latest` job with `shell: pwsh`
  that smoke-tests `conjure.ps1 --version` and confirms exit code propagation

### Tech Debt

- [x] **DEBT-01**: `ci-gate` job in `release.yml` fails with an explicit error
